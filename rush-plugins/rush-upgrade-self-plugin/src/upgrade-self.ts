...
import type { AbbreviatedPackument } from 'pacote';
...
export const upgradeSelf = async (): Promise<void> => {
  ...
  let versions: AbbreviatedPackument['versions'] | undefined;
  try {
    const { packument } = await import('pacote');
    const info: AbbreviatedPackument = await packument('@microsoft/rush', {
      preferOnline: true
    });
  ...
  const runRushJSPath: string = path.join(rushConfiguration.commonScriptsFolder, 'install-run-rush.js');
  // const rushJson: any = JsonFile.load(rushJsonPath);
  ...
  spinner = ora('Updating Rush.js related dependencies in package.json files...').start();

  const depVersionMap: Record<string, string> = {
    '@rushstack/rush-sdk': targetVersion
  };
  for (const [k, v] of Object.entries<string>(versions[targetVersion]!.dependencies!)) {
  ...
  for (const pkgJsonFile of allPackageJsonFiles) {
    try {
      ...
      terminal.writeWarningLine(`${pkgJsonFile} is not a valid JSON file, ignored`);
    }
  }

  spinner.succeed(`Updated ${changedPackageJsonFileSet.size} package.json files`);
  ...
  for (const packageJsonFile of changedPackageJsonFileSet) {
    if (packageJsonFile.includes('common/autoinstallers')) {
      ...
      try {
        Executable.spawnSync('node', [runRushJSPath, 'update-autoinstaller', '--name', autoinstallerName]);
      } catch {
